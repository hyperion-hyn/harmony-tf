...
func InvalidAddressScenario(testCase *testing.TestCase) {
	...
	senderAccount, err := testing.GenerateAndFundAccount(testCase, senderName, testCase.StakingParameters.CreateRestaking.Validator.Amount, fundingMultiple)
	if err != nil {
	...
	validatorAccount, err := testing.GenerateAndFundAccount(testCase, validatorName, testCase.StakingParameters.CreateRestaking.Map3Node.Amount, fundingMultiple)
	if err != nil {
	...
	tx, _, map3NodeExists, err := restaking.BasicCreateMap3Node(testCase, &validatorAccount, nil, nil)
	if err != nil {
	...
	tx, _, validatorExists, err := restaking.BasicCreateValidator(testCase, tx.ContractAddress, &validatorAccount, &senderAccount, nil)
	if err != nil {
