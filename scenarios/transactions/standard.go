...
func StandardScenario(testCase *testing.TestCase) {
	...
	senderStartingBalance, _ := balances.GetBalance(senderAccount.Address)
	receiverStartingBalance, _ := balances.GetBalance(receiverAccount.Address)
	...
	senderEndingBalance, err := balances.GetBalance(senderAccount.Address)
	if testCase.ErrorOccurred(err) {
		return
	}

	/*if testCaseTx.Success && testCase.Parameters.FromShardID != testCase.Parameters.ToShardID {
		logger.TransactionLog(fmt.Sprintf("Because this is a cross shard transaction we need to wait an extra %d seconds to correctly receive the ending balance of the receiver account %s ", config.Configuration.Network.CrossShardTxWaitTime, receiverAccount.Address, testCase.Parameters.ToShardID), testCase.Verbose)
	...
	logger.BalanceLog(fmt.Sprintf("Sender address: %s has an ending balance of %f  after the test", senderAccount.Address, senderEndingBalance), testCase.Verbose)
	logger.BalanceLog(fmt.Sprintf("Receiver address: %s has an ending balance of %f  after the test - expected balance is %f", receiverAccount.Address, receiverEndingBalance, expectedReceiverEndingBalance), testCase.Verbose)
