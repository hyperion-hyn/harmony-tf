...
func NonExistingScenario(testCase *testing.TestCase) {
	...
	if err != nil {
		msg := fmt.Sprintf("Failed to generate %s account", map3NodeName)
	...
	delegationTx, delegationSucceeded, err := microstake.BasicDelegation(testCase, &delegatorAccount, map3NodeAccount.Address, nil)
	if err != nil {
